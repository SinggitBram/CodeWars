...
function pickPeaks(arr) {
    ...
    for (let i = 0; i < arr.length; i++) {
        ...
        else if (arr[i - 1] === arr[i] && arr[i] === arr[i + 1] && arr[i - 2] < arr[i - 1] && arr[i + 2] < arr[i + 1]) {
            obj.pos.push(i-1)
            obj.peaks.push(arr[i-1])
        }
        else if (arr[i] === arr[i+1] && arr[i] === arr[i+2] && arr[i] === arr[i+3] && arr[i-1] < arr[i] && arr[i+4] < arr[i+3]){
            obj.pos.push(i)
            obj.peaks.push(arr[i])
        }
        else if (arr[i] === arr[i+1] && arr[i-1] <arr[i] && arr[i+2] < arr[i+1]){
            obj.pos.push(i)
            obj.peaks.push(arr[i])
        }
    }
